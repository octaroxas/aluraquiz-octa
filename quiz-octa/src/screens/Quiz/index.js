...
function WidgetResult({ results, playerName, db }) {
    ...
    return (
        <Widget>
            ...
            <Widget.Content>
                ...
                <p style={{fontFamily:'Lato', fontWeight: 900, fontSize: 20, maxWidth: 300, lineHeight:1, color: `${db.theme.colors.contastText}`}}>
                    Você acertou
...
function QuestionWidget( { question, totalQuestion, questionIndex, onSubmit, addResult,db} ){
...
function QuizScreen({ dbExternal }){
    ...
    return(
        <QuizBackground backgroundImage={dbE.bg} >
            <QuizContainer>
                <QuizLogo />
                {screenState === screenStates.LOADING && ( <WidgetLoading db={dbE} /> )}
                ...
                {screenState === screenStates.RESULT && ( <WidgetResult db={dbE} results={results} playerName={playerName} /> )}
            </QuizContainer>
        </QuizBackground>
    );
}
